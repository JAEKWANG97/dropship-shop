...
`B-101`/`B-102` 구현 계약과 `B-103`~`B-105` 상세 Planned 계약, 기존 동작의 호환 경계는 [Supplier Portal Design](docs/supplier-portal-design.md)을 기준으로 한다. Production 포털은 active 공급처 신청 개인정보 고지, 실제 이메일 delivery, `B-098` 계약 증적과 `B-100`~`B-105` gate를 모두 검증할 때까지 기본 `off`다.
