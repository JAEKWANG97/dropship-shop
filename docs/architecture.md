...
이 다이어그램은 현재 구현 기준이다. 공급처 포털의 `B-100` 신청·초대·Kakao 연결·lifecycle, `B-101` 개별 상품·검토, `B-102` 옵션 재고·24시간 예약·계좌입금 예외, `B-103` 출고 요청·최소 PII·운영 이메일 기반, `B-104` 복수 송장·수량 할당·공식 조회 링크 Web/API와 additive schema는 같은 Next.js web, Spring Boot modular monolith와 PostgreSQL 안에 구현됐다. `B-105`는 Planned 확장이며 별도 판매자 서비스, 정산 시스템 또는 결제 시스템을 만들지 않는다.
...
Supplier portal data (`B-100`~`B-104` Implemented, `B-105` Planned):
...
- shortage reports, Coreable-owned claim tasks and append-only supplier claim facts — Planned B-105; operational supplier email audit and fulfillment/product-review producers — Implemented B-103; claim-work producer — Planned B-105

V39 implements the B-100 schema extensions, V40 implements the B-101 catalog/review extensions, V41 implements the B-102 inventory, reservation, payment-command replay and received-payment exception extensions, V42 implements the B-103 delivery memo, Claim PII grant and access-log schema plus operational-email retention index, and V43 implements the B-104 plural Shipment/allocation/history expansion while preserving the expand-contract compatibility principle. B-105 claim changes remain Planned.
...
## Supplier Portal — `B-100`~`B-104` Implemented, `B-105` Planned
...
- B-100 dynamically derives supplier authority from an active user, active portal status and manager link. Implemented B-101 catalog, B-102 inventory, B-103 fulfillment and B-104 Shipment APIs scope supplier resources by both resource id and supplier id; Planned B-105 operational resource APIs must preserve the same boundary. `Supplier.status` separately gates new catalog sales/checkouts, and cross-supplier access returns `404` without revealing existence.
...
Production keeps `APP_SUPPLIER_PORTAL_ENABLED=false` by default. Implemented B-100 through B-104 do not open external routes or portal-product sale: they stay disabled, and admin commands or dispatch jobs that would create/send an activation invite fail closed, until Planned B-105, required tenant/CSRF/migration/concurrency tests, real email delivery, the new privacy disclosure and B-098 supplier contract/privacy obligations all pass their activation gates. B-104 also blocks a new admin portal-Shipment creation while disabled, after first allowing an exact stored idempotency replay; read, correction, void and delivery-evidence cleanup paths remain available. Even afterward each portal-product public read and checkout requires its Supplier to be ACTIVE with time-valid VERIFIED contract evidence; a global flag never substitutes for that per-supplier gate.
