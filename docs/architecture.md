...
이 다이어그램은 현재 구현 기준이다. 공급처 포털의 `B-100` 신청·초대·Kakao 연결·lifecycle, `B-101` 개별 상품·검토, `B-102` 옵션 재고·24시간 예약·계좌입금 예외, `B-103` 출고 요청·최소 PII·운영 이메일 기반 Web/API와 additive schema는 같은 Next.js web, Spring Boot modular monolith와 PostgreSQL 안에 구현됐다. `B-104`~`B-105`는 Planned 확장이며 별도 판매자 서비스, 정산 시스템 또는 결제 시스템을 만들지 않는다.
...
Supplier portal data (`B-100`~`B-103` Implemented, `B-104`~`B-105` Planned):
...
V39 implements the B-100 schema extensions, V40 implements the B-101 catalog/review extensions, V41 implements the B-102 inventory, reservation, payment-command replay and received-payment exception extensions, and V42 implements the B-103 delivery memo, Claim PII grant and access-log schema plus operational-email retention index without changing legacy behavior. B-104 and later shipment/claim changes remain Planned and will use expand-contract migrations.
...
/supplier/orders/:orderNumber   Implemented B-103 minimum-PII detail; Planned B-104 tracking registration
...
- B-100 dynamically derives supplier authority from an active user, active portal status and manager link. Implemented B-101 catalog, B-102 inventory and B-103 fulfillment APIs scope supplier resources by both resource id and supplier id; Planned B-104~B-105 operational resource APIs must preserve the same boundary. `Supplier.status` separately gates new catalog sales/checkouts, and cross-supplier access returns `404` without revealing existence.
