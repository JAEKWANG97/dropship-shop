...
이 다이어그램은 현재 구현 기준이다. 공급처 포털의 `B-100` 신청·초대·Kakao 연결·lifecycle, `B-101` 개별 상품·검토, `B-102` 옵션 재고·24시간 예약·계좌입금 예외, `B-103` 출고 요청·최소 PII·운영 이메일 기반, `B-104` 복수 송장·수량 할당·공식 조회 링크와 `B-105` 품절·클레임 사실 Web/API 및 additive schema는 같은 Next.js web, Spring Boot modular monolith와 PostgreSQL 안에 구현됐다. 별도 판매자 서비스, 정산 시스템 또는 결제 시스템을 만들지 않는다.
...
Supplier portal data (`B-100`~`B-105` Implemented):
...
- fulfillment channel/owner/handover additive base — Implemented B-100; portal request timestamp behavior, minimal supplier PII access logs and append-only claim access grants — Implemented B-103
- multiple shipments with item quantity allocations and correction/void/delivery evidence — Implemented B-104
- shortage reports, Coreable-owned claim tasks, append-only supplier claim facts and claim-work producer — Implemented B-105; operational supplier email audit and fulfillment/product-review producers — Implemented B-103

V39 implements the B-100 schema extensions, V40 implements the B-101 catalog/review extensions, V41 implements the B-102 inventory, reservation, payment-command replay and received-payment exception extensions, V42 implements the B-103 delivery memo, Claim PII grant and access-log schema plus operational-email retention index, V43 implements the B-104 plural Shipment/allocation/history expansion, and V44 implements the B-105 shortage/task/fact ownership and correction-chain schema while preserving the expand-contract compatibility principle.
...
/supplier/orders                Implemented B-103 fulfillment request queue
/supplier/orders/:orderNumber   Implemented B-103 minimum-PII detail and B-104 tracking registration
/supplier/shortage-reports      Implemented B-105 safe shortage report list
/supplier/shortage-reports/:reportId Implemented B-105 safe report detail without an order link
/supplier/claim-tasks           Implemented B-105 Coreable-requested safe fact task list
/supplier/claim-tasks/:taskId   Implemented B-105 fact detail, first answer and linear correction
...
/admin/shortage-reports          Implemented B-105 shortage master/detail; selected report uses ?reportId=
/admin/supplier-claim-tasks     Implemented B-105 Claim fact task queue
/admin/supplier-claim-tasks/:taskId Implemented B-105 full fact history and close
```
...
## Supplier Portal — `B-100`~`B-105` Implemented
...
- `B-103` creates the portal fulfillment request, initial `requestedAt+60일` PII cutoff and address lock in the successful deposit-confirmation transaction. It snapshots an optional max-300-character delivery memo after trim/blank-to-null normalization. There is no supplier accept/reject step.
- Implemented `B-104` expands Shipment from the legacy single record to multiple records with immutable item allocations for portal orders. Tracking registration uses `TRACKING_REGISTERED`, generates an official carrier URL from the four-code registry (`CJ_LOGISTICS`, `LOTTE`, `HANJIN`, `KOREA_POST`) and does not call a live carrier-status API. Supplier carrier/tracking correction and Coreable void/delivery-complete/guarded delivery-correction actions use 200-character single-line PII-free reasons, are idempotent, preserve evidence and recalculate the Order aggregate.
- `B-105` accepts shortage only before any tracking registration and lets suppliers answer only Coreable-created claim tasks without granting refund or claim-decision authority. The existing supplier order detail hosts whole-shortage submission only when the existing shipment-list response's `canReportShortage` is true; that capability is not part of the supplier order-detail or shortage DTO. Admin shortage uses one `/admin/shortage-reports?reportId=...` master/detail screen, and the existing admin Claim detail hosts task create/read/close in addition to the dedicated task queue.
- V44 keeps B-105 ownership inside PostgreSQL: `orders(id,supplier_id)` and `claims(id,order_id)` become composite parent keys, shortage references Order/Supplier together, task stores immutable `order_id` and references Claim/Order and Order/Supplier pairs, and fact history has one-root/one-child partial unique constraints.
- B-105 list reads stay deliberately small: no pagination, shortage `createdAt DESC, reportId DESC`, task `requestedAt DESC, taskId DESC`, `{reports:[...]}` or `{tasks:[...]}` wrappers and only the documented status/orderId/claimId filters. Pagination is deferred until measured operating volume requires it.
- Claim task instructions are four fixed requested-type/code/Korean-template mappings. Deadline is strictly after creation and at most 30 days; fact observation timestamps stay between task request and server now. Corrections form one append-only root→current-head chain. `orderDetailAvailable` exists only on supplier claim-task DTOs and reuses existing FULL/MASKED order-detail authorization; shortage report pages expose no order link.
...
- B-100 dynamically derives supplier authority from an active user, active portal status and manager link. Implemented B-101 catalog, B-102 inventory, B-103 fulfillment, B-104 Shipment and B-105 shortage/claim-task APIs scope supplier resources by both resource id and supplier id in the database predicate. `Supplier.status` separately gates new catalog sales/checkouts, and cross-supplier access returns `404` without revealing existence.
...
- Supplier cookie-authenticated unsafe methods and B-105 admin shortage approve/reject plus claim-task create/close require an exact allowlisted `Origin`; when `Origin` is absent they require an exact same-origin `Referer`, otherwise return `403`. Production cookies use `HttpOnly`, `Secure`, and `SameSite=Lax`.
...
Production keeps `APP_SUPPLIER_PORTAL_ENABLED=false` by default. Implemented B-100 through B-105 do not by themselves open external routes or portal-product sale: required tenant/CSRF/migration/concurrency checks, real email delivery, the new privacy disclosure and B-098 supplier contract/privacy obligations must all pass their activation gates. B-104 blocks a new admin portal-Shipment creation while disabled after an exact stored replay lookup. B-105 applies the same replay-first rule to new ADMIN claim-task creation: identical stored result returns, changed replay conflicts, and only a new command returns `409 SUPPLIER_PORTAL_NOT_RELEASED`; existing shortage read/review, task read/close and system close remain operational. Even afterward each portal-product public read and checkout requires its Supplier to be ACTIVE with time-valid VERIFIED contract evidence; a global flag never substitutes for that per-supplier gate.
