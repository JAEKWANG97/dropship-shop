...
Notes:
- 도매매 `qty.supplyUnit`을 고객 최소수량과 주문단위로 사용한다. 도매꾹 `qty.domeMoq`와 공급처 최대수량은 원문 증적으로만 구분해 보존한다.
- 81개 카테고리에서 카테고리당 최대 30개를 수집하며 미달은 보충 검색 없이 PASS 처리한다.

Tasks:
- [x] 공식 API와 실제 응답으로 `supplyUnit`, `domeMoq`, `supplyLoq`, 공급가 의미를 구분한다.
- [x] 수집 조건을 `mxq=10`으로 바꾸고 MOQ 1~10을 허용하며 10 초과 제외 사유를 남긴다.
- [x] 카테고리당 목표 30개, 미달 PASS, 동의어 보충 검색 없음 정책을 적용한다.
- [x] 호출 원장과 상품별 checkpoint로 일일 5,000회 제한 중단·재개를 지원한다.
- [x] 검수 결과와 manifest에 최소수량·주문단위·MOQ 분포·카테고리 부족 수량을 기록한다.
- [x] importer가 신규·기존 상품 MOQ를 멱등 저장하고 제외된 기존 상품을 숨긴다.
- [x] 정기 동기화가 MOQ 변경과 이력을 반영하고 실패 시 기존 값을 보존한다.
- [x] 수집·검수·importer self-check와 API 전체 테스트를 통과한다.
- [x] 운영 DB·이미지 백업 후 전체 수집·검수·dry-run·import를 완료한다.
- [ ] 운영 배포와 대표 MOQ 1·MOQ 2 이상 구매 흐름을 검증한다.
