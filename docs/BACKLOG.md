...
Notes:
- 고객 주문수량은 상품 최소수량 이상, 주문단위의 배수, 최대 99개여야 한다.
- MOQ 변경으로 기존 장바구니가 무효가 되어도 수량은 자동 변경하지 않는다.

Tasks:
- [x] 상품 카드와 상세 구매 패널에 MOQ·개당 가격·최소 주문 총액을 표시한다.
- [x] 상품 상세에서 수량별 총액, 인라인 오류, CTA 비활성화를 적용한다.
- [x] 장바구니 추가·합산·수정과 Checkout 직전에 MOQ를 서버에서 검증한다.
- [x] 무효 장바구니에 현재 수량·MOQ·주문단위·사유와 복구 동선을 표시한다.
- [x] Checkout 상품 영역에 단가·수량·MOQ·상품별 합계를 표시한다.
- [x] 공급처 발주 직전 최신 수량 검증 회귀를 유지한다.
- [x] Desktop 1440px과 Mobile 390px Playwright 및 실제 화면 캡처를 완료한다.
- [x] 전체 API 테스트, Web lint/build, `git diff --check`를 통과한다.
- [ ] 커밋·PR·병합 후 `main`을 동기화한다.
