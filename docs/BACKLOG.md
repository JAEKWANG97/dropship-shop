...
Notes:
- 루트 `main` worktree에 API·Web·테스트 구현이 로컬 변경으로 남아 있으며 아직 최신 `origin/main`에 반영되지 않았다.
- 새 작업을 시작하기 전에 기존 변경을 보존한 채 최신 main 기준으로 검증·PR을 마무리한다.

Tasks:
- [x] 관리자 주문 목록 API를 서버 측 페이징과 상태·검색어·주문일 필터로 전환한다.
- [x] 기존 주문 상세 query와 관리자 액션 동선을 유지한다.
- [x] API 회귀 테스트와 Playwright 관리자 주문 smoke를 갱신한다.
- [ ] 로컬 변경을 최신 `origin/main`에 안전하게 반영하고 전체 검증·PR을 완료한다.
