...
Notes:
- 검수 UI와 무관하게 판매 필수정보가 없는 상품은 backend가 `ACTIVE` 전환을 거부해야 한다.
- `HIDDEN`은 검수 대기, `ACTIVE`는 판매, `STOPPED`는 판매 안 함으로 유지한다.
...
Notes:
- 상품 수백 개를 전체 로드한 뒤 Web에서 필터링하는 현재 방식을 서버 조회로 바꾼다.
- B-064 검수 UI가 사용할 목록 조회 기반을 먼저 만든다.

Tasks:
- [ ] 관리자 상품 목록 API를 서버 측 페이징과 상태·카테고리·공급처 필터로 전환한다.
- [ ] 관리자 상품 목록 화면이 query 기반 페이지·필터 상태를 유지하게 한다.
- [ ] 공개 상품 API와 기존 관리자 상세 API에 영향이 없는지 회귀 테스트를 추가한다.
- [ ] Playwright 관리자 상품 목록 smoke를 갱신한다.
...
Notes:
- B-056의 공개 조건과 B-063의 목록 조회를 사용해 운영자가 수집 상품 검수를 끝낼 수 있게 한다.
- 일괄 공개는 검수 조건을 모두 통과한 상품에만 허용한다.

Tasks:
- [ ] `products.source_url`을 추가하고 import 스크립트가 채우게 한다.
- [ ] 상품 목록/상세에 원가·판매가·마진, 원본 링크, 옵션 수, 고시·이미지 입력 여부를 노출한다.
- [ ] 검수 플래그 계산 기준과 개별/일괄 전환 동선을 구현한다.
- [ ] B-056에서 확정한 공개 불가 사유를 관리자 화면에 표시한다.
- [ ] Playwright로 개별 검수와 공개 동선을 확인한다.
