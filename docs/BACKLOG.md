...
Notes:
- 외부 은행 API나 가상계좌 연동 전에 현재 수동 처리의 금액 무결성과 감사 기록을 먼저 강화한다.
- 구매안전서비스 제공자를 정하기 전에는 특정 은행 연동 구조를 구현하지 않는다.

Tasks:
- [ ] 입금확인 시 실제 입금자명, 실제 입금액, 입금시각, 은행 거래 메모를 필수로 기록한다.
- [ ] 실제 입금액과 결제 그룹 총액이 다르면 입금확인을 거절하고 불일치 처리로 유도한다.
- [ ] 수동 환불 완료 시 환불 은행, 계좌번호, 예금주, 이체시각, 거래 식별 메모를 필수로 기록한다.
- [ ] 입금확인과 환불 완료 증적을 관리자 주문 상세와 작업 이력에서 확인할 수 있게 한다.
- [ ] 입금자명과 환불 계좌정보는 관리자 응답에서만 노출하고 로그와 고객 알림에는 남기지 않는다.
- [ ] API 권한, validation, 중복 처리, 금액 불일치 회귀 테스트를 추가한다.
