...
Notes:
- B-020 쇼핑형 홈 위에 실제 운영 쇼핑몰다운 신뢰 정보와 구매 탐색 동기를 보강한다.
- 새 API, DB, dependency 없이 홈 화면과 공통 CSS만 수정한다.
- 실제 고객센터 정보, 사업자 정보, 법정 정책 페이지 연결은 후속 법적 고지 작업에서 처리한다.

Tasks:
- [x] `추천 상품` 문구를 `현장에서 자주 찾는 상품`으로 바꾼다.
- [x] `기본 보호구 준비`, `추락 작업 준비`, `안전 통제 구역 설치` 구매 묶음 링크 카드를 추가한다.
- [x] 홈 하단에 `배송비 포함 가격`, `세금계산서 가능`, `품절 시 배송 그룹 주문 단위 환불` 신뢰 정보를 추가한다.
- [x] `cd apps/web && npm run lint`를 통과한다.
- [x] `cd apps/web && npm run build`를 통과한다.
- [x] `git diff --check`를 통과한다.
- [x] 브라우저에서 desktop/mobile 홈 화면을 재확인한다.
