...
Notes:
- 실제 운영 기준으로 로그인 후 고객 필수 정보를 확인한다.
- 고객 회원 유형은 구분하지 않는다.
- MVP는 SMS OTP로 휴대폰 번호 소유 확인을 한다.
- NICE, KCB, Toss 인증 같은 CI/DI 본인확인은 성인인증, 중복가입 방지, 실명확인이 필요해질 때 검토한다.

Tasks:
- [ ] 로그인 후 `/account`에서 필수 정보 완료 상태를 보여준다.
- [ ] 필수 정보 항목을 이름, 연락 가능한 이메일, 휴대폰 번호 인증으로 확정한다.
- [ ] 소셜 이메일이 없거나 `oauth.local`이면 이메일 입력을 요구한다.
- [ ] `users`에 휴대폰 번호와 인증 완료 시각을 저장할 필드를 추가한다.
- [ ] SMS 인증번호 요청/확인 API를 구현한다.
- [ ] 인증번호는 hash 저장, 5분 만료, 60초 재발송 제한, 시도 횟수 제한을 적용한다.
- [ ] 실제 SMS provider 후보를 정하고 운영 env 변수 목록에 반영한다.
- [ ] 휴대폰 번호 인증 전 checkout 진입 또는 결제 진행을 막는 위치를 정한다.
- [ ] 개인정보 수집/보관 정책과 화면 고지를 갱신한다.
