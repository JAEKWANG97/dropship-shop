...
- [ ] 운영 전용 도매꾹 계정과 API key, 로그인 자격증명을 서버 secret으로 관리한다.
- [ ] 주문 직전에 상품 상태, 옵션, 수량, 공급가, 배송비를 다시 조회하고 판매 시점 snapshot과 다르면 자동 발주를 중단한다.
- [ ] 고객 입금 확인이 완료된 배송 그룹 주문만 도매꾹 발주 대상으로 전환한다.
- [ ] e-money 잔액과 실제 공급처 결제금액을 확인한 뒤 `setOrder`로 주문하고 도매꾹 주문번호와 결제 snapshot을 저장한다.
- [ ] timeout 또는 응답 유실 시 주문 목록을 대조한 뒤에만 재시도해 중복 결제를 막는다.
- [ ] 구매 주문 상태, 공급처 취소, 택배사, 송장번호를 주기적으로 동기화한다.
- [ ] 도매꾹 취소 완료와 고객 계좌환불 완료를 별도 상태와 증적으로 관리한다.
- [ ] e-money 부족, 가격 불일치, 품절, 주문 실패를 관리자 작업 큐와 알림에 연결한다.
- [ ] 고객 배송지 제공 대상에 도매꾹 공급처와 택배사를 반영하고 개인정보 처리방침·제3자 제공 고지를 갱신한다.
...
- [ ] 운영 계좌 은행명·계좌번호·예금주를 `APP_BANK_TRANSFER_*`에 설정하고 실제 입금 안내 화면을 검증한다.
