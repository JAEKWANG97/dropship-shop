...
- 현재 고객 결제 경로는 계좌입금이며 Toss Payments는 B-001로 미룬 상태다.
...
- [x] 공개 취소/환불 정책의 PG 전용 문구를 계좌입금 수동 환불 기준과 Toss 재도입 기준으로 분리한다.
...
Notes:
- 수집 상품이 대량으로 들어오는 상황에서 운영자가 검수 후 `ACTIVE` 전환하는 흐름을 관리자 화면에서 끝낼 수 있게 한다.
- 상품/주문 목록이 전체 로드 후 웹 필터 방식이라 상품 수백 개 기준으로 서버 측 페이징/필터로 전환한다.
- `HIDDEN`은 검수 대기, `ACTIVE`는 판매, `STOPPED`는 판매 안 함으로 사용한다.
- 검수 플래그와 일괄 공개 정책은 운영자 결정 후 구현하되, 판매 필수정보 없는 상품의 개별 `ACTIVE` 전환은 backend에서 막아야 한다.

Tasks:
- [ ] 관리자 상품 목록을 서버 측 페이징/상태·카테고리·공급처 필터로 전환한다.
- [ ] `products.source_url`을 추가하고 import 스크립트가 채우게 한다.
- [ ] 상품 목록/상세에 원가/판매가/마진, 원본 링크, 옵션 수, 고시/이미지 입력 여부를 노출한다.
- [ ] `ACTIVE` 전환 조건을 확정한다: 판매가, 대표 이미지, 판매 가능한 옵션, 상품 고시, 인증 대상 상품의 검수 결과.
- [ ] 확정된 `ACTIVE` 전환 조건을 backend 상태 변경 API에서 검증하고 회귀 테스트를 추가한다.
- [ ] 관리자 주문 목록도 같은 페이징 방식으로 맞춘다.
- [ ] 운영자 결정 후 검수 플래그 계산과 개별/일괄 전환 동선을 구현한다.
- [ ] Playwright 관리자 smoke를 갱신한다.
...
Tasks:
- [ ] 문의 수집 목적, 수집 항목, 보유 기간, 동의 거부권과 불이익을 `/support`에 명시한다.
- [ ] 문의 접수 시 동의 정책 버전과 동의 시각을 저장한다.
- [ ] 문의 상태(`RECEIVED`, `IN_PROGRESS`, `ANSWERED`, `CLOSED`)와 관리자 메모/답변 기록을 추가한다.
- [ ] 관리자 문의 목록에 상태 필터와 처리 동선을 연결한다.
- [ ] 비로그인 문의 접수에 기본 rate limit을 적용하고 과도한 요청을 명확히 거절한다.
- [ ] 공개 접수, 관리자 처리, 개인정보 동의 증적에 대한 API/Web 회귀 테스트를 추가한다.
...
Notes:
- B-026 실제 상품, B-056 검수 흐름, B-061 문의 화면이 준비된 뒤 최종 스크린샷 QA를 실행한다.
...
Notes:
- 기존 Toss Payments 이슈 번호는 보존한다. 현재 고객 주 결제 경로는 계좌입금이다.
- Toss를 재도입해도 계좌입금 플로우는 결제 수단 선택지로 유지한다.

Tasks:
- [ ] 카드/간편결제/PG 계좌이체/가상계좌 중 재도입 범위를 정한다.
- [ ] 가맹 신청, 구매안전서비스 이용확인증, 운영 키 준비 조건을 확인한다.
- [ ] sandbox 성공/실패/취소/중복 confirm 흐름을 실브라우저에서 검증한다.
- [ ] 결제 예외와 refund retry 운영 화면을 확인한다.
