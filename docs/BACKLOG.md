...
Notes:
- 상품별 최소주문수량과 주문단위를 1~99 범위로 관리한다.
- 기존 상품과 필드 생략 요청은 `1 / 1`을 유지한다.

Tasks:
- [x] 상품 DB 모델과 migration에 최소주문수량·주문단위를 추가한다.
- [x] 관리자 등록·수정과 공개 상품 API에 두 필드를 반영한다.
- [x] 주문수량 변경 이력과 입력값 검증을 추가한다.
- [x] 관리자 화면에서 두 값을 등록·수정·확인할 수 있게 한다.
- [x] 전체 API 테스트, Web lint/build, PostgreSQL migration smoke를 통과한다.
