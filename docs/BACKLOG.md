...
Notes:
- 고객·관리자·상품 데이터·운영 상태를 현재 배포본 기준으로 다시 검증한다.
- 운영 점검은 읽기 전용으로 진행하며 실제 주문, 입금, 개인정보 전송, 상품 상태·가격 변경은 하지 않는다.
- 각 결함은 URL, 재현 절차, 기대값/실제값, 증거, 영향 범위와 P0-P3 심각도를 남긴다.
- 합격 기준은 QA 점수 0.80 이상, P0/P1 0건, 핵심 고객 흐름 통과, 오해를 유발하는 노출 카테고리 0건이다.

Tasks:
- [x] Desktop/Mobile 고객 핵심 흐름과 검색·카테고리·필터 UX를 점검하고 스크린샷을 남긴다.
- [x] 기존 인증 세션이 있으면 관리자 회원·주문·상품·통계·알림 화면을 읽기 전용으로 점검한다.
- [x] 상품 중복, 빈 노출 카테고리, 이미지, 상태, 가격, MOQ, 옵션, 고시정보를 읽기 전용으로 감사한다.
- [x] API 전체 테스트와 Web lint/build/e2e, readiness, 콘솔·네트워크 오류를 검증한다.
- [ ] 결함을 P0-P3으로 분류하고 P0/P1은 별도 PR 단위로 수정·배포·재검증한다.
- [ ] `docs/TEST_LOG.md`와 `docs/PROJECT_LOG.md`에 최종 판정과 남은 P2/P3를 기록한다.
