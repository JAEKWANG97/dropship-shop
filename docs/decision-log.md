...
Coreable remains the customer-facing seller and keeps payment, refund, claim decision, and customer-support authority. An approved external supplier gets a bounded portal for its own products, inventory, paid fulfillment requests, tracking registration, and Coreable-requested claim facts. The portal has one active manager per supplier, a single-use email invitation, and Kakao-only invitation login. It does not include supplier settlement, a supplier acceptance step, CSV registration, or customer payment/refund access.

General supplier products that pass structural and sale-readiness checks publish automatically from the supplier's single registration action. Products flagged by certification, category, or required-information rules go to Coreable review. The supplier provides cost data while Coreable calculates the customer price with the active pricing policy. Internal draft persistence needed for multi-request editing does not create a second supplier approval-request step.

Physical deletion is limited to an abandoned supplier-portal draft that has never been submitted and has no CartItem or OrderItem reference. An option may be physically deleted only under that same never-submitted draft and must leave at least one option. Once a product has been submitted, reviewed, published, or referenced, neither it nor its options are hard-deleted; Coreable uses the existing hidden or stopped sale states instead. A general product soft-delete tombstone is not introduced. Immutable subject identifiers keep deletion audit history after the live product or option foreign key is cleared.

New portal options use `TRACKED` inventory by default and may explicitly select `UNTRACKED`; existing COREABLE options migrate as `UNTRACKED`, while portal options created between B-101 and B-102 migrate as `TRACKED/onHand=0`. Tracked inventory reserves for 24 hours, consumes on confirmation, and releases on cancellation/expiry. Normal and late processing first rechecks saleability, time-valid contract and immutable mode: that failure has priority and creates `SALE_UNAVAILABLE_AT_DEPOSIT`, even if the timestamp is also late. Only after those guards pass does a late actual timestamp or failed atomic reacquisition create `LATE_DEPOSIT_EXCEPTION`. Both paths record receipt and `PAYMENT_EXCEPTION` exactly once and never resume fulfillment.

When an identified bank-transfer receipt differs from the immutable PaymentGroup total, amount mismatch has first reason priority regardless of portal/legacy origin, saleability, or receipt time. This also covers a group already closed only by unpaid cancellation when no received Payment, Refund or Fulfillment exists. Coreable records the complete actual receipt and one `BANK_TRANSFER` Payment/PaymentGroup `PAYMENT_EXCEPTION` exactly once, releases any remaining HELD reservation without reacquisition, creates exactly one `PAYMENT_GROUP` Refund for the actual received amount with reason `PAYMENT_AMOUNT_MISMATCH`, and moves every included Order directly to `REFUND_REQUESTED`. It creates no Fulfillment, supplier notification, address lock, or supplier PII access. The order cannot resume; Coreable approves and completes the actual bank refund, after which Payment, PaymentGroup, the single Refund, and every included Order become fully refunded. A customer who still wants the products creates a new checkout.

An exact receipt found after a PaymentGroup was closed only by unpaid cancellation is also terminal when no received Payment, Refund or Fulfillment exists. Portal and legacy groups both preserve the receipt as `PAYMENT_EXCEPTION`, create one `LATE_DEPOSIT_EXCEPTION` Refund for each immutable delivery-group Order amount, and move every included Order from `CANCELLED` to `REFUND_REQUESTED`. They do not recheck stock to revive the checkout even when the bank timestamp was within the original deadline. After Coreable returns the full group total, the customer creates a new checkout.

Portal orders are exposed to the supplier immediately after admin deposit confirmation without an accept action. That transaction also consumes reservations, records the fulfillment request, and locks the address. A pre-shipment shortage report records REPORTED and hands operational ownership to Coreable without changing Order/Refund; only Coreable's later approval invokes the whole delivery-group out-of-stock/refund service.

Portal shipments are one-to-many and allocate order-item quantities. Registering carrier and tracking is recorded as `TRACKING_REGISTERED`, not proof of pickup. Customers open a server-generated official carrier URL; live carrier tracking is not part of the portal MVP. Coreable retains manual delivery correction, and an order becomes delivered only after all quantities are allocated and all shipments have delivery evidence.

Supplier order lists contain no customer PII. Detail views expose only the recipient and address fields needed for the supplier's own delivery until a stored monotonic cutoff: initialize at fulfillment request +60 days and shorten it on every tracking registration to `min(current, registeredAt+30 days)` without extending it after void/replacement. OUT_OF_STOCK, CANCELLED, REFUND_REQUESTED, and REFUNDED mask immediately regardless of Shipment presence. A Coreable-approved claim may reopen access only until an explicit deadline and while the supplier contract remains time-valid, and every detail access is audited with actor, order, reason, and time without copying PII into the log. Supplier claim input is allowed only through a Coreable-created task, is append-only fact evidence, and cannot change Claim, Order, or Refund state.

Context:

The existing system assumes Coreable manually manages non-Domeggook suppliers, stores no authoritative stock, and allows one shipment per order. The new supplier should be able to work directly without turning Coreable into a multi-seller marketplace or exposing payment and customer data beyond fulfillment needs. Existing products, orders, shipment records, and automated Domeggook fulfillment must remain compatible while the portal is introduced in reviewable slices.

Consequences:

- This decision supersedes the no-real-inventory decision for new supplier-portal options only. Existing options backfill to `UNTRACKED` and `sourceStockQuantity` stays reference metadata.
- This decision supersedes the one-shipment and tracking-input-equals-shipped rules for supplier-portal shipments only. Legacy shipment states and Domeggook tracking remain valid.
- Existing `CUSTOMER` and `ADMIN` stored roles are preserved. Supplier authority is derived from an active user, unique manager link, and active portal status so invitation acceptance does not replace an existing account role.
- Existing `Supplier.status` keeps catalog/trading meaning and gates new public sales/checkouts, while portal status independently gates manager access. A manager may finish already-paid work while sales are inactive only while the supplier contract remains time-valid VERIFIED. Contract expiry/revocation suspends portal access and hands open work to Coreable; re-verification/reactivation never restores ownership or sales silently. Other suspension, disconnection, and reactivation likewise never silently mutate or restore `Supplier.status`; the admin action explicitly chooses whether to keep or pause sales, with pause as the safe UI default.
- Existing admin supplier-work and supplier-order-completed APIs stay available for Coreable-managed and legacy orders; portal orders bypass them.
- B-102 supersedes the 2026-07-19 memo-only amount-mismatch disposition for newly processed mismatched receipts. The current memo endpoint and historical rows remain readable during expand-contract, but an identified nonzero receipt cannot be left indefinitely in `PAYMENT_PENDING` after the new command is released.
- An unreviewed application expires and is anonymized at submit+90 days; rejected application contact PII is anonymized at review+90 days. Approved contact data becomes the supplier operational record and is cleaned together with the duplicate application PII at the B-098/privacy-notice post-relationship deadline; no public withdrawal endpoint or `WITHDRAWN` application state is introduced.
- Supplier actor identifiers are not retained forever merely because an audit row is append-only. Invite/catalog/inventory/lifecycle identity follows the configured supplier-relationship retention deadline; shipment/shortage/claim identity follows its parent Order/Claim legal-retention deadline; the non-PII event evidence remains after the actor link is nulled where required.
- Draft deletion locks the Product and all of its Options, checks the same references used by cart and checkout writers, and records the deletion before removing the rows. Product-image metadata is removed in the database transaction while server-owned binary keys are placed in a durable after-commit cleanup queue, so cleanup failure never restores a deleted draft.
- Production activation is blocked until supplier email delivery and the supplier/carrier privacy disclosure are verified.
- The implementation is split into B-100 through B-105. B-097 external contact and B-098 contract/import work keep their separate user-approval boundaries.
...
## 2026-08-06: Defer Non-Selling Operations And Finish Existing Work First

Decision:

친구와 만드는 비판매 취미 서비스에서는 구매안전·홈택스 실판매 확인과 SES 실메일 발송을 진행하지 않는다. 이미 로컬 구현이 진행된 B-065를 먼저 안전하게 마무리하고 B-057 관리자 회원·알림 이력을 다음 기능으로 진행한다.

Consequences:

- B-030은 실제 고객 주문·입금을 받기 전에, B-061은 실제 이메일 알림을 사용하기로 결정할 때 다시 활성화한다.
