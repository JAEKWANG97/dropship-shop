...
## 2026-08-06 14:42 KST

- 작업: 관련 항목 B-065. 루트의 미커밋 관리자 주문 페이징 변경만 최신 `origin/main` 기반 worktree로 이식하고 API·Web·문서를 동기화했다.
- 문제·고민: 루트 `main`은 24커밋 뒤처진 채 B-065와 AWS 백업·의존성 변경 19개가 섞여 있어 그대로 rebase하거나 commit하면 다른 작업을 오염시킬 수 있었다.
- 해결방안: B-065에 속하는 API 5개, Web/E2E 5개와 API 문서만 3-way로 이식하고 기존 공통 페이지네이션 UI를 재사용했다.
- 결정: 관리자 주문 목록은 발주대기를 기본값으로 유지하며 상태·주문번호/고객 이메일·서울 기준 주문일을 서버에서 필터링하고 페이지당 기본 20건을 반환한다.
- 후속작업: PR CI와 운영 배포·smoke를 확인한 뒤 B-057 관리자 회원 관리와 알림 이력을 시작한다.

## 2026-08-06 13:02 KST

- 작업: 현재 코드·운영 AWS 상태와 사용자 합의를 기준으로 `docs/BACKLOG.md`의 Now/Next/Deferred/Later를 재정렬했다.
- 문제·고민: 완료된 B-013/B-008이 진행 중으로 남고, 스킵한 B-030/B-061이 활성 큐를 차지했으며, B-062는 이미 생성된 알람과 Budget 단계 알림·DLM 오류를 반영하지 못했다. 루트 worktree에는 B-065 구현이 미커밋 상태로 남아 있었다.
- 해결방안: B-013/B-008을 완료 이력으로 이동하고 B-030/B-061은 재개 조건이 있는 `Skipped`로 분리했다. B-062는 AWS 읽기 전용 확인 결과에 따라 갱신하고 B-065 로컬 변경은 완료로 오인하지 않도록 `Now`에 유지했다.
- 결정: 진행 중인 B-065를 먼저 최신 main에 안전하게 반영한 뒤 B-057을 시작한다. 운영 보안 B-059와 모니터링 B-062는 Next에 유지한다.
- 후속작업: 루트 worktree의 B-065 변경을 보존해 rebase·전체 검증·PR을 마무리한다. 실제 주문·입금 또는 이메일 발송을 시작하기 전에는 B-030/B-061을 다시 활성화한다.

## 2026-08-06 B-093 도매꾹 81개 카테고리 검색 정책 수집

- 관련 항목: B-093
- 결정: 81개 카테고리를 A(자동 판매 후보) 33개, R(자동 import 금지 검토 후보) 34개, M(독립 검색 비대상) 14개로 분리했다. A/R은 승인 키워드를 순서대로 사용하고 M은 목록 검색을 수행하지 않는다. 일반 작업장갑은 이번 정책의 독립 검색 대상이 아니라 참조 원본 카테고리 탐색으로만 보존한다.
- 실제 수집: API 목록 검색 229회와 상세 조회를 통해 신규 후보 944개를 확보했다. 19개 카테고리가 최대 60개에 도달했고 62개는 확보 수량으로 PASS했다. 호출 원장은 2,119회로 5,000회 일일 한도 아래였고, 수집 후 로컬 여유 공간은 약 19GiB다.
- 검토: 1,319개 로컬 수집본을 다시 선별해 `IMPORT 891 / REVIEW 68 / EXCLUDE 360 / 중복 0`을 확인했다. 처음에는 `POLICY_REVIEW_ONLY`가 다른 review 사유처럼 제외되는 분기 오류가 있었고, 하드 실패가 없으면 R군을 `REVIEW`로 남기도록 수정했다.
- 보호: 참조 상품 5개를 모두 수집했고, M군은 목록 검색 0회다. 실제 import는 dry-run(`신규 891 / 갱신 0`)으로만 수행했으며 운영 DB·카탈로그·공개 상태는 변경하지 않았다.
...
## 2026-08-06 배포 사이트 전체 운영 QA

- 관련 항목: B-095
- 문제: 81개 고정 카테고리 중 공개 상품이 없는 39개도 전역 메뉴에 노출되어 고객이 빈 목록으로 이동했고, 빈 화면에는 관리자용 등록 문구가 표시됐다.
- 해결: 새 API나 상태 저장소를 만들지 않고 기존 공개 상품 응답의 `categoryCounts`를 재사용해 전역 메뉴와 홈 추천에서 상품 1개 이상인 카테고리만 보여준다. API 실패 시에는 탐색 자체를 없애지 않고 기존 전체 메뉴를 fallback으로 유지한다.
- 판단: 현재 정책이 판매를 허용한 `PENDING` 검수 상품은 임의로 숨기지 않고 데이터 품질 후속으로 분리했다. 공급처가 다른 동일 이름·가격 상품도 자동 삭제하지 않는다.
- 검증: 운영 고객·API·데이터·인프라 read-only 점검, API 전체 테스트, Web lint/build, Desktop/Mobile Playwright 전체 회귀와 visual snapshot을 확인했다.
- 운영 반영: PR #45와 Actions run `31064199472` 배포 후 Desktop/Mobile 운영 smoke 10건, 빈 카테고리 링크와 고객용 빈 상태 문구를 재검증했다.
...
## 2026-08-06 17:54 KST

- 작업: 관련 항목 B-096. Coreable 공개 상품 999개와 81개 카테고리를 점검하고 국내 안전용품 공급 후보 29곳을 조사해 1차 3곳, 2차 14곳, 보류 5곳과 제외 7곳으로 정리했다. `docs/sourcing/safety-supplier-research.md`와 연락 우선순위 CSV를 작성했다.
- 문제·고민: 공개 도매가·MOQ·최종고객 직배송·상품 이미지 사용권을 동시에 공개한 업체가 거의 없고, 제조사 공식 소매가와 도매꾹 공급가를 같은 공급가로 오인할 위험이 있었다.
