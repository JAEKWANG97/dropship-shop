...
- Toss Payments는 B-001 후속 재도입 범위다. 재도입 시 하나의 PG 결제는 하나의 결제 그룹에 연결하고, 서버가 승인 금액·주문 상태·정책 확인·결제키 중복·상품 판매 가능성을 검증해야 한다.
- PG 승인이 발생했지만 주문을 확정할 수 없는 경우 공급처 발주를 차단하고 결제 예외·자동 취소·수동 확인 흐름을 사용해야 한다.
...
- Toss 재도입 후 PG 주문은 PG 취소/환불 성공 이후에만 환불 완료 상태가 될 수 있어야 한다.
- 환불 실행 실패 시 주문은 환불 완료 상태가 아니라 환불 실패, 재시도 필요, 또는 수동 확인 필요 상태로 남아야 한다.
- 환불 실패 건은 관리자 재시도 또는 수동 확인 큐에서 처리할 수 있어야 한다.
- 환불 기록에는 결제 수단별 실행 결과를 저장해야 한다. 계좌입금은 처리 관리자, 완료 시각, 사유와 환불 메모를, PG는 취소 요청/성공 시각, 실패 코드, 재시도 횟수, 멱등키와 거래 식별자를 저장해야 한다.
...
- 실제 계좌환불 완료 또는 PG 취소/환불 성공 없는 환불 완료, 송장 없는 배송중, 배송 기록 없는 배송완료, 배송 후 품절 전이는 금지되어야 한다.
