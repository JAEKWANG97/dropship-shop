...
- 고객 주문 내역에는 결제 성공 후 확정된 주문과 고객에게 처리 상태를 보여줘야 하는 결제 예외 주문만 노출되어야 한다.
- 결제 대기, 결제 만료, 결제 실패 상태는 일반 고객 주문 내역이 아니라 체크아웃/결제 재시도 화면에서만 다뤄져야 한다.
...
- 하나의 PG 결제는 하나의 결제 그룹(PaymentGroup)에 연결되어야 한다.
- 하나의 결제 그룹(PaymentGroup)은 여러 배송 그룹 주문을 포함할 수 있어야 한다.
...
- 결제 성공 금액과 서버 결제 그룹(PaymentGroup) 총액이 일치해야 주문을 확정해야 한다.
- 결제 성공 금액은 결제 그룹(PaymentGroup) 총액과 일치해야 한다.
- 결제 실패 또는 금액 불일치 주문은 확정하지 않아야 한다.
- 결제 승인 검증은 주문 상태, 주문 만료 여부, 주문서 정책 확인 여부, 금액, PG 결제키 중복 여부, 상품/옵션 판매 가능 상태를 모두 검증해야 한다.
- PG 결제가 승인됐지만 주문을 확정할 수 없는 경우 결제 예외로 기록하고 공급처 발주를 차단해야 한다.
- 결제 예외가 발생하면 시스템은 즉시 PG 전액 취소를 시도해야 한다.
- 결제 예외 자동 취소가 실패하면 관리자 긴급 확인 큐에 표시하고 고객에게 결제 확인 중 또는 결제 취소 처리 중 상태를 보여줘야 한다.
- 결제 이벤트는 멱등 처리와 PG 대사를 위해 원본 이벤트 단위로 기록되어야 한다.
- 결제는 서버에 생성된 결제 그룹(PaymentGroup)과 그에 포함된 `PAYMENT_PENDING` 배송 그룹 주문들을 기준으로 요청되어야 한다.
- 결제 실패, 결제 대기, 결제 만료 주문은 고객 주문 내역에 노출하지 않아야 한다.
- 단, PG 승인이 발생한 결제 예외 주문은 고객에게 처리 상태를 노출해야 한다.
...
- 반품 상품 입고가 필요한 환불은 반품 상품 입고 확인일로부터 3영업일 이내 PG 취소/환불 요청이 시작되어야 한다.
- 반환받을 상품이 없는 취소 환불은 취소 승인일로부터 3영업일 이내 PG 취소/환불 요청이 시작되어야 한다.
- 공급처 품절 주문은 환불 처리 대상으로 전환되어야 한다.
- 환불 완료 후 주문은 환불 완료 상태가 되어야 한다.
- 결제 승인 완료 주문은 PG 취소/환불 성공 이후에만 환불 완료 상태가 될 수 있어야 한다.
- PG 취소/환불 실패 시 주문은 환불 완료 상태가 아니라 환불 실패, 재시도 필요, 또는 수동 확인 필요 상태로 남아야 한다.
- 환불 실패 건은 관리자 재시도 또는 수동 확인 큐에서 처리할 수 있어야 한다.
- 환불 기록에는 PG 취소 요청/성공 시각, 실패 코드, 실패 사유, 재시도 횟수, 멱등키, PG 거래 식별자를 저장해야 한다.
...
- 결제 완료, 결제 예외, 품절, 배송 시작, 배송 완료, 지연 안내, 클레임 상태 변경, 환불 완료 알림은 `NotificationLog`로 기록되어야 한다.
- `PREPARING_SHIPMENT`은 MVP 주문 상태에서 사용하지 않아야 하며, 공급처 발주 완료 후 송장 입력 전 구간은 `SUPPLIER_ORDERED`로 표현해야 한다.
- PG 취소/환불 성공 없는 환불 완료, 송장 없는 배송중, 배송 기록 없는 배송완료, 배송 후 품절 전이는 금지되어야 한다.
...
- 푸터와 고객센터/회사 정보 페이지에는 상호, 대표자명, 사업자등록번호, 통신판매업 신고번호, 통신판매업 신고 기관, 사업장 주소, 고객센터 전화번호, 고객센터 이메일, 개인정보 보호책임자, 호스팅 제공자를 표시해야 한다.
...
- 고객 알림은 MVP에서 이메일과 주문 상세 상태 표시로 시작해야 한다.
- 문자, 카카오 알림톡, 앱 푸시는 MVP 이후 범위로 둔다.
