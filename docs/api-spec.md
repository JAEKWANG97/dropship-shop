...
- Supplier Portal: B-100 public application, one-time Kakao invitation, lifecycle and tenant session, B-101 individual catalog/review, B-102 inventory/reservation/received-payment exception, B-103 paid fulfillment/minimum-PII, and B-104 plural Shipment APIs are implemented. B-105 shortage/requested claim facts remain planned.
...
Status: `B-100` onboarding, lifecycle, Kakao activation and tenant/session endpoints, `B-101` catalog/review endpoints, `B-102` inventory/reservation/received-payment exception endpoints, `B-103` fulfillment/minimum-PII endpoints, and `B-104` plural Shipment endpoints are Implemented. The `B-098` contract-evidence endpoint and `B-105` endpoints remain Planned. Existing customer/admin legacy response shapes remain compatible.
...
- The production supplier-portal feature gate remains closed after B-104. Implemented B-102 inventory/checkout guards, B-103 fulfillment/privacy APIs and B-104 plural Shipment APIs are necessary release foundations, but customer purchase and external supplier routes remain closed until Planned B-105 plus privacy, live-email, B-098 contract, and feature-flag release gates are all ready.
...
### Web Client Surface (`B-100`~`B-104` Implemented, `B-105` Planned)

- Public, Implemented B-100: `/supplier/apply`, `/supplier/activate`
- Supplier, Implemented B-100~B-104: `/supplier`, `/supplier/products`, `/supplier/products/new`, `/supplier/products/{productId}` including the B-102 option inventory editor, `/supplier/orders`, `/supplier/orders/{orderNumber}` including B-104 Shipment registration/correction; Planned B-105: `/supplier/claim-tasks`, `/supplier/claim-tasks/{taskId}`
- Admin, Implemented B-100~B-104: `/admin/supplier-applications`, `/admin/supplier-applications/{applicationId}`, `/admin/suppliers`, `/admin/suppliers/{supplierId}`, `/admin/product-reviews`, B-102 payment-exception actions, B-103 portal takeover and B-104 portal Shipment actions in existing `/admin/orders`; B-103 Claim PII grant/access-log APIs have no dedicated Web screen; Planned B-105: supplier task create/read/close inside the existing admin Claim detail surface
