...
- `HTML` 블록은 관리자만 입력할 수 있으며, 서버 또는 렌더링 계층에서 sanitize해야 한다.
...
- HTML 상세는 XSS 위험이 있으므로 허용 태그/속성을 제한해야 한다.
- 운영 정책 고지는 상품 설명 콘텐츠와 분리해 재사용 가능한 정책 영역으로 관리해야 한다.
- 이미지 업로드는 파일 개수, 파일 크기, 확장자를 검증해야 한다.
- 업로드된 이미지는 object storage에 저장하고 DB에는 URL 또는 storage key를 저장한다.
- 주문 상품에는 가격 변경의 영향을 받지 않도록 주문 시점의 상품명, 옵션명, 단가, 수량, 금액을 스냅샷으로 저장한다.
