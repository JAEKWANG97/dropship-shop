...
- B-065 관리자 주문 목록 서버 페이징과 필터
  - API에서 주문 상태·주문번호/고객 이메일·서울 기준 주문일을 필터링하고 20건 단위 페이지 메타데이터를 반환한다.
  - 관리자 주문 화면은 필터와 선택 주문을 유지하면서 서버 페이지를 이동하고, 잘못된 페이지·크기·날짜 범위를 거절한다.
  - 기존 주문 상세와 관리자 액션을 유지했으며 API 회귀 테스트, Web lint/build와 관리자 주문 E2E로 검증했다.

- B-013 출시 전 모바일/디자인 QA 정리
  - 고객·관리자 핵심 화면을 320/390/430px와 Desktop에서 점검하고 모바일 header, 검색, 필터, 구매바, checkout, 계정·주문·관리자 화면의 overflow와 정보 밀도를 정리했다.
  - Playwright 반응형·visual regression으로 고정했고 B-095 전체 QA에서 Desktop/Mobile 회귀를 다시 통과했다.

- B-008 검색/필터 고도화
  - 전역 카테고리 탐색과 검색 결과의 관련 카테고리·가격 필터를 분리하고, 검색 조건의 `categoryCounts`로 0건 카테고리를 숨겼다.
  - 카테고리 선택은 해당 카테고리 목록으로 동작하며 고정 사이드바를 노출하지 않는다. 빈 결과와 API 실패도 고객 화면에서 구분한다.

- B-095 배포 사이트 전체 운영 QA
  - 운영 QA: 고객 Desktop/Mobile 핵심 흐름, 공개 API·상품 데이터, 인증 경계, 인프라·백업·알람을 읽기 전용으로 점검했다. 운영 관리자 인증 세션이 없어 관리자 전체 흐름은 격리 로컬 환경에서 검증했다.
  - 수정: 공개 상품 0개인 39개 카테고리를 기존 `categoryCounts`로 전역 메뉴와 홈 추천에서 숨기고, 빈 결과의 관리자용 문구를 고객 안내로 바꿨다.
  - 배포: PR #45를 병합하고 Actions run `31064199472`로 운영 반영했다. Desktop/Mobile 운영 smoke `10 passed`와 빈 카테고리 링크·홈 추천 링크 0개를 확인했다.
  - 검증: API 전체 테스트, Web audit/lint/build, 전체 Playwright `85 passed / 25 skipped / 0 failed` 2회, 운영 상품 이미지 999개와 readiness·EC2·CloudWatch·백업 상태를 확인했다.
  - 판정: QA `0.87 PASS`, P0/P1 0건. Cloudflare analytics CSP, 영문 404, B-030 고지, 의미상 중복·고시 placeholder·검수 대기 상품과 운영 MOQ 2 이상 표본 부재는 P2로 남긴다.

- B-093 도매꾹 81개 카테고리 검색 정책 수집
  - 정책: 판매 후보 A 33개, 자동 import 금지 검토 후보 R 34개, 독립 검색 비대상 M 14개로 81개 카테고리를 고정했다. 일반 작업장갑은 참조 원본 카테고리 탐색으로만 보존했다.
  - 수집: 도매꾹랭킹순(`rd`)과 `mxq=10`으로 최대 60개씩 조회해 신규 후보 944개, 목록 검색 229회를 기록했다. 19개 카테고리는 60개를 채웠고 나머지 62개는 조건을 완화하지 않고 PASS 처리했다.
  - 검토: 전체 로컬 수집본 1,319개는 `IMPORT 891 / REVIEW 68 / EXCLUDE 360 / 중복 0`이다. R군의 조건 충족 68개는 `REVIEW`로 남기고 import manifest에서 제외했다.
  - 보호: 참조 상품 5개를 모두 `REVIEW_CANDIDATE`로 보존했으며, 실제 관리자 API import·운영 카탈로그·공개 상태 변경은 실행하지 않았다.
  - 검증: collector/review/import self-check, 실제 수집, review, import dry-run(`신규 891 / 갱신 0`), `git diff --check`를 실행했다.

## 2026-08-05

- B-092 기존 후보 포함 카테고리별 최대 60개 확장 수집
  - 정책: 26개 판매 카테고리에 승인된 구체 검색어를 순서대로 적용하고 기존 수집본을 포함해 카테고리당 최대 60개만 유지한다. 필터는 완화하지 않으며 미달은 PASS 처리한다.
  - 결과: 26개 카테고리 712개를 확보했고 안전화는 60개, 나머지 25개 카테고리는 확보 수량으로 종료했다. 원본 안전장갑 카테고리는 참조 5개를 모두 포함해 56개를 별도 수집했다.
  - 보호: 동음이의어·비산업용 노이즈 33개와 상한 초과 18개를 활성 후보에서 분리했다. 전체 자동 리뷰는 `IMPORT 756 / EXCLUDE 57 / 중복 0`이며 미분류 안전장갑 55개는 판매 카테고리 확정 전까지 제외한다.
  - 운영 반영: 후속 승인 후 dry-run `UPDATE 414 / CREATE 342 / HIDE 0 / SKIP 57 / 실패 0`을 확인하고 756개를 적용했다. 반영 후 전체 상품 1,138개, 공개 ACTIVE 999개, 반영 대상 누락·상품번호 중복 0건이다.

- B-091 참조 상품 기반 원본 카테고리 확장 수집
  - 발견: 참조 상품 링크에서 공급처 원본 카테고리를 추출하고 카테고리별 도매꾹랭킹 후보를 최대 30개 수집하도록 했다.
  - 보호: 코어러블 카테고리 미매핑과 판매 조건 미충족 상품도 검토 후보로 기록하되 자동 import 예외는 만들지 않았다.
  - 검증: 예시 5개를 모두 포함한 안전장갑 후보 30개를 실제 수집했고 조건부 배송 9개와 카테고리 미매핑 30개를 보고서에서 확인했다.

- B-090 공급처 MOQ 수집·검수·정기 동기화·운영 재반영
  - 수집: 공식 API의 도매매 `qty.supplyUnit`을 고객 MOQ·주문단위로 사용하고 81개 카테고리를 최대 30개씩 수집했다. 부족한 카테고리는 보충 검색 없이 PASS 처리했다.
  - 검수·적재: `IMPORT 363 / EXCLUDE 4`, 운영 `UPDATE 178 / CREATE 185 / 실패 0`으로 반영해 공개 상품 658개와 상품번호 중복 0건을 확인했다.
  - 운영: DB·이미지 백업 후 PR #33과 배포 run `30934755570`을 완료했으며 API/Web 이미지 SHA, 내부·공개 readiness와 공개 MOQ 응답을 확인했다.
  - MOQ 검증: 실제 수집 상품은 모두 MOQ 1이라 운영 상품은 MOQ 1로 검증했다. MOQ 2 이상 흐름은 임의 운영 데이터를 만들지 않고 MOQ 6 fixture의 API·Desktop·Mobile Playwright로 검증했다.

- B-089 최소 주문수량 구매 흐름과 고객 UX/UI
  - 구매 흐름: 상품 카드·상세·장바구니·Checkout에 개당 가격, MOQ, 주문단위와 합계를 표시하고 잘못된 수량은 인라인 오류와 비활성 CTA로 차단한다.
  - 서버 보호: 장바구니 합산·수정과 Checkout 직전에 최신 MOQ를 재검증하며 기존 장바구니 수량을 자동 보정하지 않는다.
  - 운영: PR #32를 CI 통과 후 병합했으며 SSM 배포 run `30929507259`로 운영 반영했다.
  - 검증: API 전체 테스트, Web lint/build, Desktop·Mobile MOQ Playwright 6건과 실제 화면 캡처 8장을 확인했다.

- B-088 상품 최소주문수량·주문단위 모델
  - 모델: 상품에 1~99 범위의 최소주문수량과 주문단위를 추가하고 기존 상품은 `1 / 1`로 이관했다.
  - API/UI: 관리자 등록·수정·상세와 공개 상품 목록·상세 응답에 두 필드를 연결하고 변경 이력을 남긴다.
  - 운영: PR #30을 CI 통과 후 병합했고, SSH 제거로 막힌 기존 배포를 B-059 SSM run `30929167985`로 운영 반영했다.
  - 검증: Catalog API, API 전체 테스트, PostgreSQL migration smoke, Web lint/build, `git diff --check`를 통과했다.
